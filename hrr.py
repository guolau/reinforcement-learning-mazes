...
class LTM:
    "Long-term Memory"
    store = None
    ## Note - this is linux-specific -- 7/6/20, changed to suit my OS
    tmpdir = "C:\Users\Laura\Documents\MTSU Class Materials\Summer Research 2020"
    ...
    def __init__(self,N=1024,M=1,normalized=True):
        ...
        self.store[self.identity_string] = numpy.array([hrri(self.N) for x in range(self.M)])
    ...
    def lookup(self,q):
        ...
        if q not in self.store:
            self.store[q] = numpy.array([hrr(self.N,self.normalized) for x in range(self.M)])
        return self.store[q]
                    ...
                    for combination in itertools.combinations(substr,L+1):
                        ...
                        if key not in self.store:
                            ...
                            for i in range(len(combination)):
                                ...
                                subrep = numpy.array([convolve(subrep[x],combrep[x]) for x in range(self.M)])
